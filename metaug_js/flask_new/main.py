...
#cg = encdec.forward("pupper",corr_outp="doggo")[11]
#init_grads()
#backprop_gradient(cg, "logit5", gradients)
...
#def my_form_post(model=encdec):
#    model_new = copy.deepcopy(model)

#    return internal_function(model_new)

@app.route('/', methods=['POST'])
def internal_function():
    text = request.form['text']
    output = request.form['output']
    out_string, _, _, _, _, _, _, _, _, _, _, computation_graph = encdec.forward(text,corr_outp=output)
    ...
    update_params(encdec,0.01)


    preds = ",".join([to_eos(encdec.forward(pair[0])[0]) + "*" + to_eos(pair[1]) for pair in this_test_set])

    return render_template('my-form.html', data=out_string, preds=preds)
