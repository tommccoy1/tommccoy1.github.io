...
def get_loss(model, batch, criterion):
    ...
    inp, outp = batch
    #print(model(inp))
    output, logits = model(inp, outp_length = 1 + len(outp))
    seq_loss = 0
        ...
        for syll in sylls:
            ...
            for char in list(syll):
                if char in vowels:
                    ...
                    14/0
            syll_type = "".join(abstract)
            if syll_type in poss and syll_type not in inventory:
                inventory.append(syll_type)
...
def train_model_acq(model, task, max_epochs=10, lr=0.001, batch_size=4, print_every=1, patience=10, threshold=None):
    optimizer = torch.optim.Adam(model.params(), lr=lr)
    done = False
    count_since_improved = 0
    best_dev_acc = 0.0
    ...
    for epoch in range(max_epochs):
        ...
        for i, batch in enumerate(training_set):
            ...
            if i % print_every == 0:
                dev_outputs = []
                for batch in dev_set:
                    outputs, logits = model(batch[0])
                    dev_outputs.append([process_output(x) for x in outputs])

                syll_inv = syllable_inventory(dev_outputs)
                print(syll_inv)
...
def fit_task(model, task, meta=False, train=True, test=True, lr_inner=0.01, batch_size=100, update_embeddings=True):
