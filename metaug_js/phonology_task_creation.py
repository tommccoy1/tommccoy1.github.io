...
# Creates a phonology task
def make_task_acq(n_test_batches=1000):
    ...
    for i in range(n_test_batches):
        for ur in ["V", "VC", "CV", "CVC"]:
            sr = "." + ur + "."
            steps = ["next" for _ in range(len(ur))]

            candidate = output_string(ur,sr, v_list, c_list, steps=steps)
    ...
    return pairs, dev_pairs, [], vocab, [v_list, c_list, [2,3,1,0]]
